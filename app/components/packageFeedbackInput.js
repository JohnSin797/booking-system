...
export default function PackageFeedbackInput ({ item, getData }) {
    ...
    const submitReply = async (e) => {
        try {
            e.preventDefault()
            await csrf()
            await axios.post('/api/package-feedback-reply/store', {package_feedback_id: item.id, user_id: user.id, reply: reply})
            .then(res=>{
                setReply('')
                getData()
    ...
    return (
        <section>
            <form className='border border-slate-900 p-3 space-y-3' onSubmit={submitReply}>
                <p className='font-bold flex gap-2 items-center'><span>{item?.user?.name}</span> <StarRate star={item?.user?.package_rating?.stars} /></p>
                <p>{item?.comment}</p>
                    ...
                    item?.package_feedback_reply?.map((rep,id)=>{
                        return (
                            <div key={id} className="border border-slate-900 p-2 space-y-2">
                                <p className="font-bold flex gap-2 items-center"><span>{rep?.user?.name}</span> <StarRate star={rep?.user?.package_rating?.stars} /></p>
                                <p>{rep?.reply}</p>
